...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.innerWidth <= 768 && heroSlider) {

    heroSlider.addEventListener('click', function () {

      // 1. Obtener la box activa
      const activeBox = document.querySelector('.hero-box.active');

      if (!activeBox) return;

      // 2. Encontrar el botón dentro de esa box
      const heroBtn = activeBox.querySelector('.btn-hero');

      if (!heroBtn) return;

      // 3. Alternar clase
      heroBtn.classList.toggle('btn-activo');
    });
  }
});